...
def load_sub_davis(path='YSDT/utils/davis_subset.txt'):
    with open(path, mode='r') as f:
        subset = set(f.read().splitlines())
    return subset


def load_sub_yv(path='YSDT/utils/yv_subset.txt'):
    with open(path, mode='r') as f:
        subset = set(f.read().splitlines())
    return subset


def load_referring_yv_val(path='YSDT/utils/referring-youtubevos-val.txt'):
    with open(path, mode='r') as f:
        subset = set(f.read().splitlines())
    return subset
