...
from YSDT.dataset.utils import im_normalization
from YSDT.inference.inference_core import DEVAInferenceCore
from YSDT.inference.result_utils import ResultSaver


def get_input_frame_for_YSDT(image_np: np.ndarray, min_side: int) -> torch.Tensor:
...
def flush_buffer(YSDT: DEVAInferenceCore, result_saver: ResultSaver) -> None:
    # process all the remaining frames in the buffer
    cfg = YSDT.config
    new_min_side = cfg['size']
    need_resize = new_min_side > 0

    for frame_info in YSDT.frame_buffer:
